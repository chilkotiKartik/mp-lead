        ...
        {states.map((s) => (
          <button
            ...
            className={`rounded-full border px-4.5 py-2 text-[13px] font-semibold transition-colors cursor-pointer ${
              active === s ? "bg-ink text-paper border-ink" : "border-line hover:border-ink"
            }`}
          >
            {s}
          </button>
        ...
        {visible.map((f) => (
          <motion.div key={f.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <Link href={`/fellows/${f.id}`} className="group block">
              <div
                className="relative aspect-3/4 rounded-[3px] overflow-hidden flex items-end transition-[border-radius,transform] duration-500 group-hover:rounded-br-[46px] group-hover:-translate-y-1"
              ...
              >
                ...
                <div className="relative p-4.5 text-white">
                  <h4 className="font-serif font-medium text-[17px] mb-0.5">Fellow — {f.batch}</h4>
